...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', message: 'Restyle backend is running', timestamp: new Date().toISOString() });
});
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ token: jwt.sign({ id: user.id }, process.env.JWT_SECRET, 
             { expiresIn: '7d' }), user: { id: user.id, name: user.name, email: user.email } 
    });
  } catch (err) {
...
app.get('/api/clothing', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query( 'SELECT * FROM clothing_items WHERE user_id = $1 ORDER BY created_at DESC',
      [req.user.id]);
...
app.post('/api/clothing', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `
      INSERT INTO clothing_items (user_id, name, category, color, formality, season, notes)
      VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [
        req.user.id,
        cleanText(name, 'Unnamed Item'),
        normalizeCategory(category),
        cleanText(color).toLowerCase(),
...
app.post('/api/predict-clothing', authenticateToken, upload.single('image'), async (req, res) => {
  if (!req.file) return res.status(400).json({ ok: false, error: 'NO_FILE', message: 'No image file provided' });
  try {
    console.log(`[PREDICT] File received: ${req.file.originalname} (${req.file.size} bytes)`);
    console.log(`[PREDICT] Calling Python predictor...`);
    const raw = await callPredictService(req.file.buffer, req.file.originalname, req.file.mimetype);
...
app.delete('/api/clothing/:id', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query( 'DELETE FROM clothing_items WHERE id = $1 AND user_id = $2 RETURNING *',
      [req.params.id, req.user.id]
    );
...
app.post('/api/upload-clothing', authenticateToken, upload.single('image'), async (req, res) => {
  ...
  if (!category || !color) {
    ...
    return res.status(400).json({ error: 'Missing required fields: category, color'});
  }
  try {
    ...
    const combinedNotes = [ brand ? `Brand: ${brand}` : null, size ? `Size: ${size}` : null, notes || null]
      .filter(Boolean).join(' | ');

    const result = await pool.query(
      `INSERT INTO clothing_items (user_id, name, category, color, formality, season, notes, image_url)
    ...
    res.status(201).json({ success: true, clothing_item: result.rows[0], imageUrl});
  } catch (error) {
...
app.post('/api/outfits/generate', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const outfits = beamSearchGenerateOutfits(items, { formality, season, beamWidth});
    res.json({ totalItems: items.length, totalOutfitsGenerated: outfits.length, outfits});
  } catch (error) {
